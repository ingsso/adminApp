...
본 프로젝트는 실시간 장애물 근접 경고 앱)과 연동되는 관리자용 모니터링 앱입니다.

지역(기관) 관리자가 담당 구역을 설정하면, 해당 구역의 Geohash를 기반으로 Firebase에 등록된 장애물(CCTV) 탐지 내역을 카카오맵에서 시각적으로 모니터링하고, 실시간 푸시 알림(FCM)을 수신할 수 있습니다.
